...
class Transformation:
    ...
    def apply(self, transformations: dict, active: bool) -> dict:
        ...
        """
        if active:
            transformations.update(self._get_transformation_params())
        if self.next_transformations:
            return self.next_transformations.apply(transformations, active)
...
class TransformationGenerator:
    ...
    def __init__(self):
        self.transformations_chain = GrayscaleTransformation(
            CircularTransformation(BlurTransformation(CropTransformation()))
        )
...
class CloudinaryService(IcloudinaryService):
    ...
    async def transform_image(
        ...
        try:
            ...
            transformed_url = (
                eager_transformations[0].get("secure_url")
