...
async def test_check_first_user_admin(client, db_session):
    ...
    assert (
        user_from_db.role == RoleSet.admin
    ), f"Expected role 'admin', but got {user_from_db.role}"
...
async def test_second_user_role_user(client, db_session):
    ...
    assert (
        user_from_db.role == RoleSet.user
    ), f"Expected role 'user', but got {user_from_db.role}"
...
async def test_access_for_not_admin(client, db_session):
    ...
    user_from_db_next = await crud_users.get_user_by_email(
        another_user["email"], db_session
    )
    ...
    ), "Non-admin user should not be able to deactivate another user"
