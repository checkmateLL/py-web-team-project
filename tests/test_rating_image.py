...
async def test_added_rates(client, db_session):
    ...
    assert (
        rating_from_db.value == 4
    ), f"Expected rating value 4, but got {rating_from_db.value}"
    ...
    assert (
        image_from_db_after.average_rating != old_average_rating
    ), "Average rating did not change"
    expected_new_average_rating = 4.0
    assert (
        image_from_db_after.average_rating == expected_new_average_rating
